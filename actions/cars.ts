...
import { serializeCarData, SerializedCar } from "@/lib/helpers";
...
// AI analysis output for a car image
export type CarImageAnalysis = {
  make: string;
  model: string;
  year: number;
  color: string;
...
// Input needed to create a car (aligned with schema fields used here)
export type CarCreateData = {
  make: string;
  model: string;
  year: number;
  price: any; // Decimal-like; serialized by Prisma. Keep as any to avoid coupling to Decimal.js in actions
  mileage: number;
  color: string;
  fuelType: string;
  transmission: string;
...
): Promise<ActionResult<CarImageAnalysis>> {
  try {
    ...
    try {
      const carDetails = JSON.parse(cleanedText) as CarImageAnalysis;

      // Validate the response format
      const requiredFields: (keyof CarImageAnalysis)[] = [
        "make",
        "model",
        "year",
        "color",
...
export async function getCars(
  search: string = ""
): Promise<ActionResult<SerializedCar[]>> {
  try {
...
// Delete a car by ID
export async function deleteCar(id: string): Promise<ActionResult<void>> {
  try {
    const { userId } = await auth();
    if (!userId) throw new Error("Unauthorized");
